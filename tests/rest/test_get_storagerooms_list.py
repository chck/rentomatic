...
def test_get_failed_response(mock_use_case, client):
    ...
    assert json.loads(http_response.data.decode('UTF-8')) == \
        {'type': 'SystemError', 'message': 'test message'}
    assert http_response.status_code == 500
    assert http_response.mimetype == 'application/json'


@mock.patch('rentomatic.use_cases.storageroom_use_case.StorageRoomListUseCase')
def test_request_object_initialization_and_use_with_filters(mock_use_case, client):
    mock_use_case().execute.return_value = res.ResponseSuccess([])

    internal_request_object = mock.Mock()

    with mock.patch('rentomatic.use_cases.request_objects.StorageRoomListRequestObject') as mock_request_object:
